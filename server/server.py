...
class Server:
    ...
    def setLoggingInfo(self, loggingInfo):
        if loggingInfo["enable"]:
            self.loggingPath = os.path.abspath(
                os.path.join(self.initialDirectory, loggingInfo["path"])
            )
        else:
    ...
    def logError(self, logInfo):
        if self.loggingPath is not None:
            ...
            logFile.write(f"{datetime.datetime.now()}: ERROR {logInfo}\n")
            logFile.close()

    def logInfo(self, logInfo):
        if self.loggingPath is not None:
            logFile = open(self.loggingPath, "a")
            logFile.write(f"{datetime.datetime.now()}: INFO {logInfo}\n")
            logFile.close()
    ...
    def setAuthorizationInfo(self, authorizationInfo):
        if authorizationInfo["enable"]:
            ...
            for file in authorizationInfo["files"]:
                self.filesWithRestrictedAccess.append(
                    os.path.abspath(os.path.join(self.initialDirectory, file))
                )
        else:
    ...
    def handleUsername(self, commandSegments, commandSocket, clientAddress,
                       threadInfo):
        if len(commandSegments) != 1:
            commandSocket.send("501 Syntax error in parameters or arguments.".encode())
            self.logError(
                f"Client ({clientAddress[0]},{clientAddress[1]}) entered a "
                f"command with wrong syntax."
            )
        elif (
            threadInfo["user"] is not None
            or threadInfo["enteredUsernameForLogin"] is not None
        ):
            commandSocket.send("503 Bad sequence of commands.".encode())
            self.logError(
                f"Client ({clientAddress[0]},{clientAddress[1]}) "
                f"attempted to login while he/she was already signed in or "
                f"already entered his/her username."
            )
        else:
            ...
            self.logError(
                f"Client ({clientAddress[0]},{clientAddress[1]}) "
                f"entered a username."
            )

    def handlePassword(self, commandSegments, commandSocket, clientAddress,
                       threadInfo):
        if len(commandSegments) != 1:
            commandSocket.send("501 Syntax error in parameters or arguments.".encode())
            self.logError(
                f"Client ({clientAddress[0]},{clientAddress[1]}) "
                f"entered a command with wrong syntax."
            )
        else:
            if threadInfo["enteredUsernameForLogin"] is not None:
                userWithEnteredUsername = self.findUserByUsername(
                    threadInfo["enteredUsernameForLogin"]
                )
                if (
                    userWithEnteredUsername is not None
                    and userWithEnteredUsername.password == commandSegments[0]
                ):
                    commandSocket.send("230 User logged in, proceed.".encode())
                    threadInfo["user"] = userWithEnteredUsername
                    self.logInfo(
                        f"User '{userWithEnteredUsername.username}' logged in!"
                    )
                else:
                    commandSocket.send("430 Invalid username or password.".encode())
                    self.logError(
                        f"Client ({clientAddress[0]},{clientAddress[1]}) "
                        f"entered wrong authentication info."
                    )
                ...
                self.logError(
                    f"Client ({clientAddress[0]},{clientAddress[1]}) "
                    f"attempted to enter password before username."
                )

    def handlePrintingWorkDirectory(self, commandSocket, threadInfo):
        commandSocket.send(("257 " + threadInfo["currentDirectory"]).encode())
        self.logInfo(
            f"User '{threadInfo["user"].username}' got his/her current working "
            f"directory ({threadInfo["currentDirectory"]}). "
        )

    def accessDenied(self, user, filename):
        return (
            self.hasAuthorizationSystem
            and user.isAdmin is False
            and filename in self.filesWithRestrictedAccess
        )
    ...
    def makeFile(self, commandSocket, filename, threadInfo):
        ...
        if self.accessDenied(user, fileAddress):
            ...
            self.logError(
                f"User '{user.username}' attempted to remove a restricted file "
                f"and was prevented."
            )
        else:
            try:
                ...
                file.close()
                commandSocket.send(f"257 {filename} created.".encode())
                self.logInfo(
                    f"File '{filename}' was created by user '{user.username}'."
                )
            except:
                commandSocket.send("500 Error.".encode())
                self.logError(
                    f"User '{user.username}' attempted to create a file but "
                    f"failed."
                )
    ...
    def makeDirectory(self, commandSocket, directoryName, threadInfo):
        ...
        try:
            ...
            commandSocket.send(f"257 {directoryName} created.".encode())
            self.logInfo(
                f"Directory '{directoryName}' was created by user "
                f"'{user.username}'."
            )
        except:
            commandSocket.send("500 Error.".encode())
            self.logError(
                f"User '{user.username}' attempted to create a directory but "
                f"failed."
            )

    def handleMakingDirectoryOrFile(self, commandSegments, commandSocket,
                                    threadInfo):
        if len(commandSegments) == 2 and commandSegments[0] == "-i":
            ...
            commandSocket.send("501 Syntax error in parameters or arguments.".encode())
            self.logError(
                f"User '{threadInfo["user"].username}' entered a command with "
                f"wrong syntax."
            )
    ...
    def removeDirectory(self, commandSocket, directoryName, threadInfo):
        ...
        try:
            ...
            commandSocket.send(f"250 {directoryName} deleted.".encode())
            self.logInfo(
                f"Directory '{directoryName}' was deleted by user "
                f"'{user.username}'."
            )
        except FileNotFoundError:
            commandSocket.send("550 No such file or directory.".encode())
            self.logError(
                f"User '{user.username}' attempted to delete a non-existing "
                f"directory."
            )
        except OSError:
            if os.path.isdir(os.path.join(currentDirectory, directoryName)):
                ...
                self.logError(
                    f"User '{user.username}' attempted to delete a non-empty "
                    f"directory."
                )
            else:
                commandSocket.send("500 Error.".encode())
                self.logError(
                    f"User '{user.username}' attempted to delete an existing "
                    f"file with a command for deleting directories."
                )
    ...
    def removeFile(self, commandSocket, filename, threadInfo):
        ...
        if self.accessDenied(user, fileAddress):
            ...
            try:
                ...
                commandSocket.send(f"250 {filename} deleted.".encode())
                self.logInfo(
                    f"File '{filename}' was deleted by user '{user.username}'."
                )
            except:
                commandSocket.send("500 Error.".encode())
                self.logError(
                    f"User '{user.username}' attempted to delete a directory "
                    f"but failed."
                )

    def handleRemovingDirectoryOrFile(self, commandSegments, commandSocket,
                                      threadInfo):
    ...
    def handleListingAllFilesInADirectory(self, commandSocket, threadInfo):
        ...
        for file in allFilesInCurrentDirectory:
            if not self.accessDenied(user,
                                     os.path.join(currentDirectory, file)):
        ...
        self.logInfo(
            f"User '{user.username}' got all Files in his/her current directory"
            f" which he/she had access to ({allFilesInCurrentDirectory})."
        )

    def handleChangingDirectory(self, commandSegments, commandSocket,
                                threadInfo):
        user, currentDirectory = threadInfo["user"], threadInfo["currentDirectory"]
        if len(commandSegments) == 0:
            ...
            self.logInfo(
                f"User '{user.username}' changed its working directory to "
                f"server's initial directory ({self.initialDirectory})"
            )
        elif len(commandSegments) == 1:
            directory = commandSegments[0]
            newDirectory = os.path.abspath(os.path.join(currentDirectory,
                                                        directory))
            if os.path.exists(newDirectory):
                ...
                self.logInfo(
                    f"User '{user.username}' changed its working directory to "
                    f"{self.initialDirectory}"
                )
            else:
                commandSocket.send("550 No such file or directory.".encode())
                self.logError(
                    f"User '{user.username}' attempted to change its directory "
                    f"to non-existing directory."
                )
        else:
            commandSocket.send("501 Syntax error in parameters or arguments.".encode())
            self.logError(
                f"User '{user.username}' entered a command with wrong syntax."
            )
    ...
    def sendAlertEmail(self, user):
        alert = (
            f"Your usage limit became {user.size} which is less than our ftp "
            f"server's usage threshold to warn the user which is "
            f"{self.usageLimitWarningThreshold}."
        )
        endOfDataIdentifier = "\r\n.\r\n"
        ...
        base64EncodedAuthInfo = base64.b64encode(
        ...
        mailSocket.send(f"RCPT TO: <{user.email}>\r\n".encode())
        response = mailSocket.recv(1024).decode()
        if response[:3] != "250":
            raise Exception("250 reply not received from server.")
        ...
        mailSocket.send(f"to: {user.email}\r\n".encode())
    ...
    def handleFileDownload(self, commandSegments, commandSocket, threadInfo):
        ...
        if len(commandSegments) != 1:
            ...
            fileAddress = os.path.abspath(os.path.join(currentDirectory,
                                                       filename))
            if self.accessDenied(user, fileAddress):
                commandSocket.send("550 File unavailable.".encode())
                self.logError(
                    f"User '{user.username}' attempted to download a restricted "
                    f"file and was prevented."
                )
                ...
                try:
                    ...
                    if user.hasUsageLimit and user.size < fileSize:
                        ...
                        self.logError(
                            f"User '{user.username}' couldn't download his/her "
                            f"desired file due to his/her usage limit."
                        )
                        self.sendData("No Data", threadInfo["clientToken"])
                    else:
                        if user.hasUsageLimit:
                            ...
                            if (
                                user.alert
                                and user.size < self.usageLimitWarningThreshold
                            ):
                                try:
                                    self.sendAlertEmail(user)
                                    self.logInfo(
                                        f"User '{user.username}' was alerted "
                                        f"about reaching his usage limit below "
                                        f"threshold."
                                    )
                                except Exception as error:
                                    print("Error In Sending e-mail:", error)
                                    self.logError(
                                        f"Failed to alert user '{user.username}' "
                                        f"about reaching his/her usage limit "
                                        f"threshold."
                                    )
                        self.sendData(fileContent, threadInfo["clientToken"])
                        commandSocket.send("226 Successful Download.".encode())
                        self.logInfo(
                            f"User '{user.username}' successfully downloaded "
                            f"file '{filename}'."
                        )
                except FileNotFoundError:
                    commandSocket.send("550 No such file or directory.".encode())
                    self.logError(
                        f"User '{user.username}' attempted to download a "
                        f"non-existing file."
                    )
                    self.sendData("No Data", threadInfo["clientToken"])
    ...
    def handleHelp(self, commandSocket, threadInfo):
        ...
        commandsHelpText += (
            "PWD, It is used for printing server's current working directory\n"
        )
        ...
        self.logInfo(
            f"User '{threadInfo["user"].username}' successfully received "
            f"commands help."
        )
    ...
    def handleQuit(self, commandSocket, threadInfo):
        ...
        self.logInfo(f"User '{user.username}' successfully logged out.")
    ...
    def parseCommand(self, command, commandSocket, clientAddress, threadInfo):
        ...
        if commandSegments[0] == "USER":
            self.handleUsername(
                commandSegments[1:], commandSocket, clientAddress, threadInfo
            )
            ...
            if threadInfo["user"] is None:
                ...
                if commandSegments[0] == "PWD":
                    ...
                    self.logError(
                        f"User '{threadInfo["user"]}' entered a non-available "
                        f"command."
                    )
    ...
    def run(self):
        ...
        while True:
            ...
            threading.Thread(
                target=self.communicateWithClient,
                args=([commandSocket, clientAddress])
            ).start()
